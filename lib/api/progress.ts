...
type LegacyConceptLedgerResponse = {
  data: Array<LegacyConceptLedgerEntry | ConceptLedgerEntry>
  meta?: {
    total?: number
    page?: number
  }
...
function normalizeConceptLedger(
  ...
  return {
    data: (response.data ?? []).map((entry) => {
      const legacy = entry as LegacyConceptLedgerEntry
      const normalized = entry as ConceptLedgerEntry
      return {
        conceptId: normalized.conceptId ?? legacy.id,
        conceptTitle: normalized.conceptTitle ?? legacy.title,
        attempts: entry.attempts ?? 0,
        passRate: entry.passRate ?? 0,
        avgScore: normalized.avgScore ?? entry.passRate ?? 0,
        reviewEase: entry.reviewEase ?? legacy.ef ?? 2.5,
        nextReviewDate: normalized.nextReviewDate ?? legacy.nextReview ?? new Date(0).toISOString(),
...
export async function getProgressInsights(
  ...
  const qs = new URLSearchParams({ range })
  appendRoadmap(qs, roadmapId)
  return apiClient<ProgressInsights>(`/api/mastery/insights?${qs.toString()}`, {
    headers,
  })
}
